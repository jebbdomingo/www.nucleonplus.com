...
/**
 * Used in the QBO employee sync controller
 *
 * @author  Jebb Domingo <https://github.com/jebbdomingo>
 */
class ComNucleonplusControllerBehaviorEmployeesyncable extends KControllerBehaviorAbstract
{
    /**
     *
     * @var array
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            ...
            'reference_column' => array('EmployeeRef'),
            'controller'       => 'com:nucleonplus.controller.employeeaccount',
        ));
    ...
    public function getData(KModelEntityInterface $entity)
    {
        ...
        if (is_array($this->_reference_column))
        {
            foreach ($this->_reference_column as $column)
            {
                if ($entity->{$column})
                {
                    $data['EmployeeRef'] = $entity->{$column};
                    break;
                }
            }
        }
        elseif ($entity->{$this->_reference_column}) {
            $data['EmployeeRef'] = $entity->{$this->_reference_column};
        }

        if (!isset($data['EmployeeRef'])) {
            $data['EmployeeRef'] = '#' . $entity->id;
        }
